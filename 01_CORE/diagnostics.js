...
async function init(){
 ...
 sites=await readJSON('ecosystems.json');$('#results').innerHTML=sites.map(e=>`<article class="diagnostic-row" id="test-${e.id}"><div><h2>${esc(e.name)}</h2><p>${esc(e.layer.provider)} · ${esc(e.layer.method)}</p><p class="diagnostic-info">${esc(e.layer.edition)}</p></div><span class="diagnostic-status">NOT TESTED</span></article>`).join('');
}
$('#run').onclick=async()=>{
